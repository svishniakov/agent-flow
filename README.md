...
- Verification Readiness Gate runs before workers in architecture-gated lane runs: `verification-readiness.json` and `verification_readiness` prove selected `risk_gates` and `verification_gates` are ready, or stop as `paused-blocked` after a user decline with `resume_phrase=Готово`.
...
Simplicity Scope Coverage prevents smoke/docs-only closure. For positive architecture-gated worker runs, `lane-map.json` records `engineering_simplicity_scope.primary_surfaces`, optional `secondary_surfaces`, and evidence. Workers fill `scope_coverage`; QA writes `Engineering Simplicity Scope`; reviewer `Contract Drift` rejects peripheral-only closure; final `Engineering Simplicity` names every primary surface.

Lane Boundary Evidence Gate proves that successful worker lanes changed product code only inside their assigned file boundaries. Workers record `boundary.allowed_paths`, optional `boundary.forbidden_paths`, and `changed_paths_artifact`; `scripts/record-lane-boundary.py` writes `checks/lane-boundary-<lane-id>.json`. The validator checks repo-relative POSIX `changed_paths` with `fnmatch.fnmatchcase`: every path must match `allowed_paths`, and `forbidden_paths` wins. QA `Architecture Invariants`, reviewer `Contract Drift`, and final `Boundary Evidence` mention every worker lane id.

Verification Readiness Gate runs after approved architecture design and before workers. `lane-map.json` records `verification_readiness`, and `verification-readiness.json` covers every selected `risk_gates` and `verification_gates` facet. If readiness needs a documented local command, the status becomes `needs-approval`; the agent asks the user and runs only an approved documented safe command, then records `approval_executions` and repeats readiness. If the user declines, the current pass stops as `paused-blocked`, final verdict is `blocked`, and `final.md` includes the manual instruction plus `resume_phrase: Готово`. Workers may start only after the latest readiness attempt is `ready`, and QA writes `Verification Gate Results`.
...
Simplicity Scope Coverage не даёт закрыть gate только smoke/docs-правкой. Для positive architecture-gated worker runs `lane-map.json` записывает `engineering_simplicity_scope.primary_surfaces`, опциональные `secondary_surfaces` и evidence. Workers заполняют `scope_coverage`, QA пишет `Engineering Simplicity Scope`, reviewer `Contract Drift` отклоняет peripheral-only closure, а final `Engineering Simplicity` называет каждый primary surface.

Lane Boundary Evidence Gate доказывает, что успешные worker lanes меняли product code только внутри назначенных файловых границ. Worker записывает `boundary.allowed_paths`, опциональные `boundary.forbidden_paths` и `changed_paths_artifact`; `scripts/record-lane-boundary.py` пишет `checks/lane-boundary-<lane-id>.json`. Validator проверяет repo-relative POSIX `changed_paths` через `fnmatch.fnmatchcase`: каждый path должен попасть в `allowed_paths`, а `forbidden_paths` всегда сильнее. QA `Architecture Invariants`, reviewer `Contract Drift` и final `Boundary Evidence` называют каждый worker lane id.

Claim Evidence Gate закрывает разрыв между текстом и доказательством. Для positive architecture-gated runs Architecture Contract в секциях `QA Gates` и `Reviewer Checklist` задаёт строки `Claim Evidence`, а `claim-evidence.json` связывает каждый claim id с `owner_lane`, reviewer lane, статусом `supported`, subjects, evidence path и буквальными `markers`. Если claim остаётся `gap` или marker не найден в evidence-файле, `ship` и `pass-with-risks` блокируются.

Verification Readiness Gate идёт после approved architecture design и до workers. `lane-map.json` записывает `verification_readiness`, а `verification-readiness.json` покрывает каждый выбранный facet из `risk_gates` и `verification_gates`. Если для проверки нужен документированный локальный запуск, статус становится `needs-approval`: агент спрашивает пользователя и запускает только одобренную documented safe command, затем пишет `approval_executions` и повторяет readiness. Если пользователь отказывается, текущий проход сразу останавливается как `paused-blocked`, финальный вердикт становится `blocked`, а `final.md` содержит ручную инструкцию и `resume_phrase: Готово`. Workers стартуют только после последней попытки со статусом `ready`, а QA пишет `Verification Gate Results`.
