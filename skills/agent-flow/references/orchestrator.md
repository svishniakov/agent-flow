...
- Apply the Astra-Sol model policy and shared instructions from `SKILL.md`. The main agent starts at Astra `high` and may reach `xhigh` for coordination or architecture complexity established after checking requirements, context, and tools; `agents/orchestrator.md` configures only its helper. Documenter and QA use Astra `high` with an `xhigh` ceiling. Plan reviewer normally uses Sol `high`; independent final `reviewer.qa` is a separate assignment selected with `--role reviewer --trigger qa-critical` at Sol `xhigh`.
- Reasoning upgrade is not the default fix. Diagnose requirements, context, tools, implementation, tests, and the plan first; revise architecture when the cause requires it. Before an authorized increase, record the matching role trigger (or the root's coordination/architecture reason), supporting evidence, previous and target levels, session link, and attempt count. No repeated user approval is needed. Keep the exact model ID, service tier, assignment_id, scope, context, and attempt count unchanged. A specialist works under a separate role and does not grant additional recovery attempts; renaming an assignment does not reset them. Ordinary fixable test failures do not start the entire Blocked Recovery Path.
- Preserve the reached reasoning level on every continuation. The resolver is stateless and cannot retain assignment history; its later default output is not permission to downgrade. Repeated triggers cannot exceed the role ceiling, and an already reached ceiling is not another escalation. Confirm the host applied the selected settings before dependent execution. If a supported explicit continuation needs a new session of the same model, stop the previous execution, transfer complete working context, and preserve assignment_id, both session IDs, ownership, and attempts. Do not use silent close/resume drift as escalation. If the path or application cannot be confirmed, stop dependent work with a precise blocker.
- Preserve original user requirements and acceptance criteria, accepted and superseded decisions, unknowns, edit boundaries, completed changes, check results, forbidden repeats, and accessible evidence in every handoff. Mutually consistent new specs and tests do not prove the original contract was met. Historical A/B/C settings and journals do not change when the active profile changes.
