...
- QA and reviewer fill Acceptance Criteria Traceability Gate and Surface Evidence Gate data in `acceptance-traceability.json`: each `Acceptance Criteria` id gets source, requirement, subjects, `supported` or `gap`, `surface_expectations`, evidence paths, literal `markers`, and matching `surface`/`polarity`/`proof_kind` evidence.
- Contract Negative Fixture Gate data also lives in `acceptance-traceability.json`: any `gate`, `cli`, `query`, `storage`, `config`, or `parser` record gets `negative_fixture_evidence` with a negative or drift fixture path, literal marker, and no `polarity=positive`.
