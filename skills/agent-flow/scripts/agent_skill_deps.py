...
def target_paths(name: str, source_type: str) -> tuple[str, str]:
    ...
    return (f"~/.agents/skills/{slug}", f".agents/skills/{slug}")
...
def install_instructions(name: str, source_type: str) -> tuple[str, str, str | None, str]:
    ...
    return (
        ...
        None,
        "Manual source; this registry has no authoritative public source yet. Skills CLI installs normally use ~/.agents/skills; legacy Codex installs under ~/.codex/skills are still scanned.",
    )
...
def detect_target(explicit_target: str) -> str:
    ...
    for global_root in (home / ".agents" / "skills", home / ".codex" / "skills", home / ".claude" / "skills"):
        global_skill = global_root / "agent-flow"
        if root == global_skill:
            return "global"
        try:
    ...
    for index, part in enumerate(parts[:-2]):
        if part in {".agents", ".codex"} and parts[index + 1] == "skills" and parts[index + 2] == "agent-flow":
            return "project"
    return "global"
