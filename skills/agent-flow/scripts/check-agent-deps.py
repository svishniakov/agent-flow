...
def prompt_report(
    ...
    report = build_report(roles, installed, registry, scope="core", target=target)
    full_report = build_report(roles, installed, registry, scope="full", target=target)
    summary = report["summary"]
    full_summary = full_report["summary"]
    heading = "Agent Flow post-install dependency check" if post_install else "Agent Flow dependency check"
    print(heading)
    print(f"Agent Flow target: {summary['target']}")
    print(f"Missing core skills: {summary['missing']}")
    print(f"Missing full skills: {full_summary['missing']}")
    print("")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--prompt", action="store_true", help="Show check-only post-install guidance.")
