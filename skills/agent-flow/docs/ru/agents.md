...
Пользователь не выбирает, запускать ли субагентов. Agent Flow может запускать их автоматически, если оркестратор решил, что делегирование даст полезную evidence или параллельную работу, и в текущей среде есть инструмент для запуска subagents.
...
Вспомогательный оркестратор Agent Flow. Помогает основному агенту выбрать flow, внутренний budget, trace policy, порядок работы, результат Dependency Gate и критерии проверки. Он готовит delegation packets, интегрирует handoffs, следит за Definition of Done и проверяет, что финальный ответ опирается на evidence.

Не имеет права сам включать Agent Flow или придумывать публичные режимы. Субагентов запускает только после внутреннего выбора workflow, прохождения Dependency Gate и проверки пользы от делегирования.
...
Независимый reviewer. Ищет реальные риски: баги, регрессии, недостающие проверки, несоответствие плану, security/data-loss concerns и release blockers. Используется после реализации или перед релизом. Если есть architect-owned review contract, сверяет diff с этим contract перед verdict о готовности.
