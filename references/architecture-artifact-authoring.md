...
- QA and reviewer fill Acceptance Criteria Traceability Gate data in `acceptance-traceability.json`: each `Acceptance Criteria` id gets source, requirement, subjects, `supported` or `gap`, evidence paths, and literal `markers`.
- Contract Negative Fixture Gate data also lives in `acceptance-traceability.json`: any `gate`, `cli`, `query`, `storage`, `config`, or `parser` record gets `negative_fixture_evidence` with a negative or drift fixture path and literal marker.
