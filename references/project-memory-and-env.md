...
If `lessons.md` is missing, do not invent one during normal work. Create or update it only when the user reports a process failure, a repeated blocker appears, or the task explicitly asks to record a lesson.
...
After a user correction or repeated process failure, update project `lessons.md` with:
