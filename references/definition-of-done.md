...
- Acceptance Criteria Traceability Gate is covered for positive architecture-gated runs: Architecture Contract `QA Gates` and `Reviewer Checklist` list `Acceptance Criteria` ids, `acceptance-traceability.json` maps every id to source, requirement, subjects, `supported` status, evidence paths, and literal `markers`; any missing id, missing marker, or `gap` blocks `ship` and `pass-with-risks`;
- Contract Negative Fixture Gate is covered for contract-like acceptance items: any `gate`, `cli`, `query`, `storage`, `config`, or `parser` acceptance record has `negative_fixture_evidence` with at least one evidence path and literal marker for a negative or drift fixture;
...
- Acceptance Criteria Traceability Gate blocked `ship` and `pass-with-risks` until every Architecture Contract `Acceptance Criteria` id had `acceptance-traceability.json` evidence paths and literal markers.
- Contract Negative Fixture Gate blocked `ship` and `pass-with-risks` until every `gate`, `cli`, `query`, `storage`, `config`, or `parser` acceptance record had negative or drift fixture marker evidence.
