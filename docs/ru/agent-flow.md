...
Agent Flow - это скилл для Codex, который помогает вести сложную задачу от запроса до проверенного результата. Он не включается автоматически. Пользователь должен поставить в начало prompt один из префиксов: `Agent Flow`, `$agent-flow` или `agent-flow`.

Главная идея скилла простая: пользователь запускает Agent Flow одним ведущим префиксом, а оркестратор сам выбирает подходящий маршрут. Он удерживает границы задачи, собирает нужный контекст, под капотом переключает внутренние budgets, решает, нужны ли субагенты, выполняет работу и проверяет результат перед финальным ответом.
