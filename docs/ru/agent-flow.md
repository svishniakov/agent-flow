...
- подготовить delegation packet для субагентов, если budget и форма задачи оправдывают делегирование;
...
- `light` budget не запускает субагентов.
- `standard` и `release` могут запускать субагентов, когда оркестратор может обосновать дополнительную проверку или параллельную работу.
...
Для больших PRD или релизных задач Agent Flow может разделить работу на lanes: implementation, integration, QA и review, если выбранный budget разрешает субагентов. Это внутренний workflow pattern, а не публичный режим и не обход правила: `light` остаётся solo.
...
- выбран `standard` или `release`, либо пользователь явно попросил subagents, spawn, multi-agent или delegation;
