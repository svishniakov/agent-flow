...
`light` budget не запускает субагентов. В `standard` и `release` Agent Flow может запускать их, если оркестратор решил, что делегирование даст полезную evidence или параллельную работу, и в текущей среде есть инструмент для запуска subagents.
...
Не имеет права сам включать Agent Flow или придумывать публичные режимы. Не запускает субагентов для `light`.
