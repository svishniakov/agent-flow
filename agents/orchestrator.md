...
- Enforce Harness Evaluation Loop after gates produce a learning trigger: write `harness-evaluation.json`, record `learning_triggers`, findings, proposals, source evidence, final `Harness Evaluation`, and reviewer `Harness Evaluation Review` for positive lane-map runs. Keep proposals signal-only with `requires_human_approval=true`; do not auto-edit Architecture Matrix, capability registry, role prompts, Golden Trace Runs, or project memory.
