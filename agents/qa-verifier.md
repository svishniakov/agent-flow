...
- When Acceptance Criteria Traceability Gate applies, create or update `acceptance-traceability.json`: every `Acceptance Criteria` id you own must name source, requirement, subjects, `supported` or `gap`, evidence paths, and literal `markers`.
- When Contract Negative Fixture Gate applies, add `negative_fixture_evidence` for every owned `gate`, `cli`, `query`, `storage`, `config`, or `parser` acceptance item; the evidence must be a negative or drift fixture with literal markers.
...
- Acceptance Criteria Traceability Gate evidence, including `acceptance-traceability.json`, `Acceptance Criteria` ids, `markers`, `supported`, and any `gap`
