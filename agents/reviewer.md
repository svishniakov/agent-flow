...
- When Acceptance Criteria Traceability Gate applies, verify `acceptance-traceability.json`: every required `Acceptance Criteria` id must have source, requirement, subjects, `supported` status, evidence paths, and literal `markers` found in evidence files; reject positive verdicts for any `gap` or missing marker.
- When Contract Negative Fixture Gate applies, verify every `gate`, `cli`, `query`, `storage`, `config`, or `parser` acceptance item has `negative_fixture_evidence` with marker-backed negative or drift fixture evidence.
...
- Acceptance Criteria Traceability Gate coverage for `acceptance-traceability.json`, `Acceptance Criteria`, `markers`, `supported`, and any `gap`
- Contract Negative Fixture Gate coverage for `negative_fixture_evidence`, `gate`, `cli`, `query`, `storage`, `config`, and `parser`
